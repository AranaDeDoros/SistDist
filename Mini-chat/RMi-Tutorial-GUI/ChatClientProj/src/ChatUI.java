...
public class ChatUI{
  ...
  public void doConnect(){
	    if (connect.getText().equals("Conectar")){
	    	if (name.getText().length()<2){JOptionPane.showMessageDialog(frame, "Ingresa un nombre."); return;}
	    	if (ip.getText().length()<2){JOptionPane.showMessageDialog(frame, "Ingresa una IP."); return;}	    	
	    	try{
	    	...
	    	}catch(Exception e){e.printStackTrace();JOptionPane.showMessageDialog(frame, "ERROR....");}		  
	      }else{
	    	  	updateUsers(null);
  ...
  public void sendText(){
    if (connect.getText().equals("Conectar")){
    	JOptionPane.showMessageDialog(frame, "Conectate primero."); return;	
    }
  ...
  public ChatUI(){
	    ...
	    top.add(new JLabel("Tu nickname: "));top.add(name);    
	    top.add(new JLabel("Servidor: "));top.add(ip);
	    top.add(connect);
